...
import static edu.aku.wasimabbas.ehsas_evaluation.utils.CreateTable.SQL_CREATE_SCHOOLS;
...
import edu.aku.wasimabbas.ehsas_evaluation.models.Schools;
...
//        String[] whereArgs = new String[]{"%" + spDateT.substring(0, 8).trim() + "%"};
    ...
    public Cursor getRecords() {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from schools where semisCode not in(select B4 from form where istatus = 1)", null);
        return res;
    }
